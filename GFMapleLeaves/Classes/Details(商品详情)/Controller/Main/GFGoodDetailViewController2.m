...
#import "GFGoodDetailViewController2.h"
...
@interface GFGoodDetailViewController2 ()<UIScrollViewDelegate>
...
@implementation GFGoodDetailViewController2
...
{
    //判断回到顶部按钮是否隐藏
    _returnBtn.hidden = (scrollView.contentOffset.y > ScreenH-50) ? YES : NO;
}
...
//    self.navigationItem.leftBarButtonItem = [UIBarButtonItem ItemWithImage:SETIMAGE(@"hoem_Details_Upper left corner_return") WithSelected:SETIMAGE(@"hoem_Details_Upper left corner_return") Target:self action:@selector(returnToHomeVC)];
//    self.navigationItem.leftBarButtonItem = [UIBarButtonItem ItemWithImage:[UIImage imageNamed:@"hoem_Details_Upper left corner_return"] WithHighlighted:[UIImage imageNamed:@"Details_Btn_More_normal"] Target:self action:@selector(toolItemClick)];

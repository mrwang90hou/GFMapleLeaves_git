...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:LOGINOFFSELECTCENTERINDEX object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        weakSelf.selectedViewController = [weakSelf.viewControllers objectAtIndex:GFTabBarControllerHome]; //默认选择商城index为1
    }];
...
- (void)viewDidLoad {
    ...
    self.selectedViewController = [self.viewControllers objectAtIndex:GFTabBarControllerHome]; //默认选择商城index为1
}
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController {
    ...
    if(viewController == [tabBarController.viewControllers objectAtIndex:GFTabBarControllerPerson]){
